...
def main(config, template_mesh):
    ...
    for i in tqdm(range(len(df))):
        img_name = df.at[i, 'subDirectory_filePath']

        input_img, proc_param, img = preprocess_image(os.path.join(config.AffectNet_path, 'validation', img_name))
        ...
        if config.save_flame_parameters:
            ...
            # np.save(config.out_folder + '/params/' + img_name + '.npy', flame_parameters_)
            np.save(config.out_folder + '/params/' + img_name + '.npy', flame_parameters[0][3+config.pose_params+config.shape_params:])
    #
